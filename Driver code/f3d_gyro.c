//Authors: Nathaniel Ferguson and Yin Liping
...
void f3d_gyro_interface_init() {
  ...
  GPIO_InitStructureAF.GPIO_Pin = GPIO_Pin_5 | GPIO_Pin_6 | GPIO_Pin_7;
  ...
  GPIO_Init(GPIOA,&GPIO_InitStructureAF);
...
void f3d_gyro_write(uint8_t* pBuffer, uint8_t WriteAddr, uint16_t NumByteToWrite) {
  ...
  //Setting chip to high - cuts off data transfer
  GYRO_CS_HIGH();
}
...
/*gets the data readings from the gyro*/
