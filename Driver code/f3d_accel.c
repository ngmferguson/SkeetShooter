...
float inline f3d_accel_calculate_pitch(float *accel_data){
  return atan2(accel_data[0], sqrtf(pow(accel_data[1], 2) + pow(accel_data[2],2)));
}

float inline f3d_accel_calculate_roll(float *accel_data){
  return atan2(accel_data[1], sqrtf(pow(accel_data[0], 2) + pow(accel_data[2],2)));
}
